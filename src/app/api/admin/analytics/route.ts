...
export const GET = withAuth([UserRole.ADMIN])(async (req) => {
  const totalFarmers = await prisma.user.count({ where: { role: UserRole.FARMER } });
  const totalVets = await prisma.user.count({ where: { role: UserRole.VET } });
  const pendingVets = await prisma.user.count({ where: { role: UserRole.VET, status: 'PENDING' } });
